...
2. **Thumbnail** — `CachedNetworkImage(imageUrl: reel.thumbnailUrl, fit: BoxFit.cover, fadeInDuration: Duration.zero)`. Renders the moment bytes arrive (fast — the matching precache from `_warmAhead` has already populated the disk cache for this index, so a scroll-back paints from disk with no network hit). Fully covers the gradient.
...
| Backend runtime | Node 18 + Express | Smallest possible footprint for the spec's six endpoints (`/health`, `/reels`, `/series/:id`, `/progress/:id` x2, `/progress/bulk-sync`, `/continue-watching`). No ORM — raw SQL via a thin driver wrapper keeps the monotonic-upsert logic visible in one file (`repositories/progressRepo.js`) so a reviewer can audit the conflict-resolution rule in 30 seconds. Layered structure (config / middleware / repositories / routes) keeps `server.js` ~50 lines so the composition is easy to scan. |
| Backend DB | `better-sqlite3` (default) with `pg` adapter | SQLite means a reviewer can `npm i && npm run init-db && PEXELS_API_KEY=... npm run seed` and have a working backend in under a minute — no docker, no Postgres install. The `pg` adapter is included so the same server file works against Postgres unchanged for a real deployment. |
...
- Series-level "download all" button — per-episode download is implemented (the spec line is "Download controls (per-episode **or** full-series)"). The parallel "delete all for this series" sweep button does ship — adding the symmetrical "download all" would be a 30-line BLoC event + button.
